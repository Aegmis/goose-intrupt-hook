#!/usr/bin/env bash
#
# Example Aegmis policies for the goose PreToolUse hook.
#
# The hook POSTs an approval with:
#   tool_name  = "developer__shell" | "developer__text_editor"
#   action     = "bash_command" | "edit_file"
#   tool_kwargs = { "command": ... }                       for developer__shell
#                 { "command": <subcmd>, "path": ..., ... } for developer__text_editor
...
#
# NOTE: for developer__text_editor, tool_kwargs.command is the SUB-command
# (write/str_replace/insert/…); the file lives in tool_kwargs.path. The hook
# already lets "view" (read) through before any policy is consulted.
#
...
create_policy '{
  "name": "goose-destructive-shell",
  "description": "Any rm — rm -rf and plain rm <file> — plus dd, mkfs",
  "trigger_tool_names": ["developer__shell"],
  "conditions": {
    "logic": "AND",
    "rules": {
...
create_policy '{
  "name": "goose-protect-secrets",
  "description": "text_editor writes/edits to .env, secrets, or prod config",
  "trigger_tool_names": ["developer__text_editor"],
  "conditions": {
    "logic": "AND",
    "rules": {
